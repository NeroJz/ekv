	...
	class Marking extends MY_Controller 
	{
		...
		******************************************************************************************/
		function index() 
		{
			...
			$totalSem = $this->config->item("total_sem");
	        ...
	        $output['output'] = $this->load->view('marking/v_evaluation_form', $data, true);
		...
		******************************************************************************************/
		...
		/******************************************************************************************
		 * Description		: this function to get class by lecture assign and modul in the class
		* 					  json responder
		* input				: -
		* author			: UMAIRAH
		* Date				: 14 MARCH 2014
		...
		******************************************************************************************/
		function spsubject_kelas()
		{
			...
			$class_data = $this->m_subject->get_classes_akhir($courselect, $semester, $cm_id_modul);
		...
		******************************************************************************************/
		function save_configurations()
		{
			...
			$academikOpen = $this->m_subject->isMarkingOpen("PAK", $semselect);
			$amaliOpen = $this->m_subject->isMarkingOpen("PA", $semselect);
			$teoriOpen = $this->m_subject->isMarkingOpen("PT", $semselect);
			...
			if($academikOpen || $amaliOpen || $teoriOpen) // salah satu open, so load detail pelajar
			{
					...
					$data['primaryKeyList'] = $semselect."%".$courselect."%".$subselect.":".$tempcategory.":".$cmID."%".$kelas;
			...
			$data['kelasID'] = $kelas;
			$data['kelas'] = $this->m_subject->get_classes_akhir($courselect, $semselect, $cmID);
		...
		******************************************************************************************/
		function load_configurations()
		{
			...
			$tempcategory = $this->input->post("tempKAt"); //temp buang lepas wujud fucntion AK
			...
			$pelajar = $this->m_pelajar->student_marking1($cmID, $semselect, $kelas);
		...
		* Modification Log	: 
		******************************************************************************************/
		function get_assignmentByStud()
		{
			$semselect = $this->input->post("semesterP");
			$cmID = $this->input->post("ksmid2");
			$kelas = $this->input->post("slct_kelas");
			$param = $this->input->post("aParam");
			
			$data['senaraipelajar'] = $this->m_pelajar->student_by_assmnt_n_studID($cmID, $assmnt_id, $semselect,$kelas);			
			
			$aJson= array("pelajar" => $data['senaraipelajar']);			
			echo(json_encode($aJson));
		}
		...
		******************************************************************************************/
		function save_assignment()
		{
			...
			$pelajar = $this->m_pelajar->student_marking1($cmid, $semester, $kelas);
		...
		******************************************************************************************/
		function get_ak_swb()
		{
			...
			$type		 = $this->input->post("type");
			$kelas       = $this->input->post("slct_kelas");
			//$data['weightage'] 	= $this->m_subject->paper_percentage($pentaksiran, $cm_id);
			
			//$papers =  $this->m_subject->get_all_paper(5, 'P');
			//$papers =  $this->m_subject->get_all_paper($cm_id, $pentaksiran);			
		...
		******************************************************************************************/
		...
		/******************************************************************************************
		* Description		: this function process donlod excel
		* input				: -
		* author			: 
		* Date				: 
		* Modification Log	: umairah - class/t marking
		******************************************************************************************/
...
$j++;
		...
		/******************************************************************************************
		* Description		: this function to donlod markah keseluruhan
		* input				:  
		* author			: 
		* Date				: 
		* Modification Log	:umairah
		******************************************************************************************/
		function export_xls_markahStudent_keseluruhan(){
			...
			$courselect = $this->input->get("kursusid");
			$subselect = $this->input->get("subjectid");
			$semselect = $this->input->get("semesterP");
			$pentaksiran = $this->input->get("pentaksiran");
			$mptid = $this->input->get("mptID");
			$kelas = $this->input->get("kelas3");
			$tempcategory = $this->input->get("tempKAt2"); //temp buang lepas wujud fucntion AK
			...
			$this->excel->getActiveSheet()->setCellValue('A1', 'Markah Pentaksiran Akhir Pelajar');
