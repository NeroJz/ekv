<legend><h3>Senarai KUPP Kolej</h3></legend>
...
check = false;
		...
		function reAssignTable()
		{
			$(".btn_edit_director").bind("click",function(){
				...
				jAjaxLm("#span_result",site_url + "management/user/edit_kupp/" + user_id,'');
		    });
		}

	</script>
	<!-- Button to trigger modal -->
	<a href="#myModal" role="button" class="btn btn-primary" data-toggle="modal">+ KUPP Baru</a><br>
	...
	$this -> table -> set_heading('id','Nama Pengguna', 'Username', 'Email', 'Tarikh Daftar', 'Tarikh Log Masuk', 'Status', 'No Telefon', 'Kolej', 'Jawatan', 'Tindakan');
	foreach ($query  as $item) {
		...
		$this -> table -> add_row('',$item['user_name'], $item['user_username'], $item['user_email'], $created_on, $last_login, $status,$item['phone'], $item['col_name'],$item['ul_name'], '<a title="'.$item['user_id'].'" href="#edit" id="btn_edit_director" name="btn_edit_director" data-toggle="modal"><i class="icon-edit"></i></a>');
	}
	...
	<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  		<div class="modal-header">
    		...
    		<h3 id="myModalLabel">Tambah Pengarah Baru</h3>
  		</div>
  		<div class="modal-body">
    		<form name="form_add_director" method="post" action="<?=site_url('management/user/add_user') ?>">
    			<table>
    				<tr>
    					<td>Nama Pengguna</td>
    					<td>:</td>
    				...
    				<tr>
    					<td>Jawatan</td>
    					<td>:</td>
    					<td><input type="text" name="jawatan" id="jawatan" value="8" /><span class="label label-success">KUPP</span></td>
    				</tr>
