...
<?php //$gred_akademik; ?>
<legend><h3>Pencantuman Markah Akademik Mengulang</h3></legend>
<center>
<form id="frm_pusat" action="<?= site_url('examination/combine_repeat/view_list')?>" method="post" >
...
if(isset($pilihan))
{
	if('ak' == $pilihan)
	{
        ...
        if('cc' == $pentaksiran){
			?>
			<table cellpadding="0" width="100%" style="width:100% !important;" cellspacing="0" border="0" class="display" id="pelajar_akademik_cantum">
            <thead>
                <tr>
                    <th>Bil</th>
                    <th>Nama Murid</th>
					 	...
					 	foreach($subjek_akademik as $rsa){
							...
							if($mVal1 == "-" || $mVal2 == "-"){
								...
								if(empty($aMarkS["mt_full_mark"]))
								{
									...
									storeMarkAndSetGradeRepeat($aMarkS["md_id"], $tempMark, $aMarkS["marks_assess_type"]);
                    ...
                    <td align="center" bgcolor="#CCCCCC"><span id="cur_gred" style="text-align:center;"><?=($jum_markah[$j]>= -5)?$jum_markah[$j]:"T"?></span>
						...
						foreach($subjek_akademik as $rsa){
							...
							if($jum_markah[$j] == "-"){
								...
								$sgred_cur[$rsa->mod_id] = getGradeRepeat($jum_markah[$j], $aMark["marks_assess_type"],'grade_type');								
							}
							?>
