...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			$user = $this->where($this->config->item('identity', 'ion_auth'), $identity->user_username)->users()->row();  //changed to get_user_by_identity from email
			...
			if ($user)
			{
				...
				else
				{
					...
					//$message = "test email je ".$this->config->item('admin_email', 'ion_auth');
					$this->email->clear();
	...
	public function logout()
	{
		...
		if (file_exists('./assets/application.appcache') && file_exists('./assets/studentRegistration.appcache')) 
		{
			//fopen update appcache version
			$file = fopen("./assets/application.appcache", "r") or exit();
			...
			if($file)
			{
				//push all line into array
				while(!feof($file))
				{
					$x++;
					if($x == 2)
					{
						array_push($aLine, "# ".time().$iduser.PHP_EOL); // PHP_EOL which produces \r\n or \n 
						fgets($file);
					}
					else if($x != 2)
					{
						array_push($aLine, fgets($file));						
					}
				}
			}
			...
			$file2 = fopen("./assets/studentRegistration.appcache", "r") or exit();
			...
			if($file2)
			{
				...
				while(!feof($file2))
				{
					...
					if($y == 2)
					{
						array_push($aLine2, "# ".time().$iduser.PHP_EOL); // PHP_EOL which produces \r\n or \n 
						fgets($file2);
					}
					else if($y != 2)
					{
						array_push($aLine2, fgets($file2));						
					}
				}
			}
		...
		//Output rewrite the application.appcache
		$file = fopen("./assets/application.appcache", "w") or exit();
		if($file)
		{
			foreach($aLine as $row)
			{
		...
		$file2 = fopen("./assets/studentRegistration.appcache", "w") or exit();
		if($file2)
		{
			foreach($aLine2 as $row)
			{
				fwrite($file2, $row);
			}
		}
	...
	public function in_group($check_group, $id=false, $check_all = false)
	{
		...
		else
		{
			...
			{
				$groups_array[$group->id] = $group->ul_name;
			}
