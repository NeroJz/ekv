...
class Ion_auth_model extends CI_Model
{
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db->select('user_id, user_password, salt')
	...
	public function forgotten_password($identity)
	{
		...
		$key = $this->hash_code($activation_code_part.$identity->salt);
		...
		//print_r($identity);
		//die($this->identity_column);

		if($this->identity_column === "user_username")
			$identityValue = $identity->user_username;

		$this->db->update($this->tables['users'], $update, array($this->identity_column => $identityValue));
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if ($this->identity_column == 'user_email' && $this->email_check($email))
		{
			$this->set_error('account_creation_duplicate_email');
			return FALSE;
		}
		elseif ($this->identity_column == 'user_username' && $this->username_check($username))
		{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', user_username, user_email, user_id, user_password, active, last_login')
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.ul_type,'.$this->tables['groups'].'.ul_name, '.$this->tables['groups'].'.ul_description')
		                ->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
		                ->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.ul_id')
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->db->where($this->tables['groups'].'.ul_id', $id);
		}
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'tahun' => $aSesiConfig['tahun'],
		    'sesi' => $aSesiConfig['sesi']
		);
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', user_id, user_username, user_email, last_login')
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->db->get_where($this->tables['groups'], array('ulname' => $group_name))->num_rows();
		if($existing_group !== 0)
		{
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->db->get_where($this->tables['groups'], array('ul_name' => $group_name))->row();
		...
		if (is_string($additional_data)) $additional_data = array('ul_description' => $additional_data);
		...
		$this->db->update($this->tables['groups'], $data, array('ul_id' => $group_id));
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db->delete($this->tables['groups'], array('ul_id' => $group_id));
