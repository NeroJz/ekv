	...
	var callback = function()
	{
		...
		if(-1 != val.indexOf("trAdd"))
		{
			...
			if("" != assgmnt_id1)
			{
				...
				//{
					var request = $.ajax({
						url: site_url+"/examination/markings/assignmentToDelete",
						type: "POST",
...
function fnOpenAssignment(val)
{
	...
	var request = $.ajax({
		url: base_url+"index.php/examination/markings/get_assignment",
		type: "POST",
...
$(document).ready(function(){
	...
	$('#slct_kursus').change(function()
	{
		...
		{
			var request = $.ajax({
				url: site_url+"/examination/markings/spsubject",
				type: "POST",
	...
	$('#btn_config_markP').click(function()
	{
		...
		if("dahbuka" == temptarikh)
		{
			...
			{
				var request = $.ajax({
					url: site_url+"/examination/markings/get_swb",
					type: "POST",
				...
				request.done(function(data)
				{
					...
					if(null != data.weightage)
					{
						...
						else
						{
							...
							'id="tugasanterbaik0" class="validate[required] span7"/></td>'+
							'<td>&nbsp;</td>'+
							'</tr>'
	...
	$('#tblAssgnMarks').on('change','.cellMarks',function(event)
	{
		...
		if(avgCal/mark_assg_selection != -99.99)
		{
			...
			if(markahs <= mark_total_assg)
			{
				...
				.val(new Number(markahs).toFixed(2))
	...
	$('#formAssgMark').on('click', '#btnSaveAssgMarks', function() 
	{
		...
		$('#assgmnpop').block({ 
			message: '<h5><img src="'+base_url+'assets/img/loading_ajax.gif" alt="Sedang process"/>Sedang diprocess, Sila tunggu...</h5>', 
			css: { border: '3px solid #660a30' } 
		});
			...
			if(markschanged.length>0)
			{
				...
				request.done(function(data)
				{
					...
					$('#tblStudent > tbody  > tr').each(function()
					{
						...
						if(index!=count)
							$(this).find(".ttlmrks").html(new Number(ttl).toFixed(2));
						else
	...
	{	
		if($('#slct_jubject').val().length > 0)
		{
			...
			var request = $.ajax({
				url: site_url+"/examination/markings/get_ak_swb",
				type: "POST",
				data: {pntksrn : pentaksiran, cmID : cm_id, type : sb_cat},
				dataType: "json"
			});
